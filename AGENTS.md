...
- In the canonical ecommerce example, `pricing/apply_discount` refreshes to `function.arithmetic_leaf.monotone_down_nonnegative.v1`, `pricing/apply_tax` refreshes to `function.arithmetic_leaf.monotone_up.v1`, and truthful `pricing/calculate_total` refreshes to `function.wrapper.pipeline.v1`. M20 adds no new supported function family.
- M20 exposes explicit unsupported-function truth through exactly `semantic_review.support_status`, `semantic_review.unsupported_reason_codes`, and `semantic_review.rewrite_hints`. New supported reviews write `support_status: supported`; unsupported function reviews write `support_status: unsupported`.
- Consumers should branch on `semantic_review.support_status == "unsupported"`, not on `verdict` or `evaluator_scope`. Legacy reviews without `support_status` still fall back to `evaluator_scope` plus `unsupported.*.v1` compatibility-key inference.
