...
- [ ] **spec_error_to_json_entry emits empty path for Io/Json errors** — SpecError::Io and SpecError::Json produce `"path": ""` in JSON output. An AI agent iterating errors[].path receives an empty string. Fix: make path Optional<String> with skip_serializing_if = Option::is_none. Effort: XS. (Added by /ship 2026-04-06)
- [ ] **spec test single-file module filter uses only last output path segment** — cargo_test_filter_for uses output.file_name() which gives only the last segment. Nested outputs (src/generated/spec) produce wrong filter prefix. Effort: XS. (Added by /ship 2026-04-06)
- [ ] **stale detection bypassed by passports without contract_hash** — Units written by spec generate/build (no contract_hash) are always shown as "valid" in spec status, never stale, even after contract changes. Known architectural issue: spec generate and spec build should not overwrite contract_hash written by spec test. See also: spec build/generate overwrites passport evidence. (Added by /ship 2026-04-06)
