...
- [ ] **M24: promote `function.arithmetic_leaf.monotone_up.v1` as the second real leaf-family packet** — Use the same harness-shaped workflow on the monotone-up family to prove the leaf promotion path is not specific to nonnegative clamp semantics. The M23 maintainer smoke gate above is already explicit and green; M24 can now start from that branch truth instead of re-litigating packet-diff ambiguity.
