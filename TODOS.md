...
- [ ] **M5: Golden JSON fixture tests for --format json stability** — Committed fixture files under `spec-cli/tests/fixtures/` (validate-valid.json, validate-invalid.json, status-valid.json, status-stale.json). Integration tests in cli.rs diff actual `spec validate --format json` / `spec status --format json` output against them. Any unintentional shape change = test failure. Required before M5 ships. Effort: XS. Depends on: M5 --format json implementation. (Added by /plan-eng-review + codex outside voice 2026-04-06)
