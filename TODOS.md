...
- [ ] **Make generated molecule tests warning-clean under strict consumers** — The canonical ecommerce example currently compiles and tests cleanly in the documented loop, but `RUSTFLAGS='-D warnings' cargo test --manifest-path examples/ecommerce/Cargo.toml` fails on an unused import in `examples/ecommerce/src/generated/pricing/molecule_tests.rs`. Root cause: `spec-core/src/generator.rs` treats `.test.spec` `covers` as "symbols to import" and emits `use` lines for every covered unit, but `covers` is a semantic exercised-units list, not a guarantee that the Rust body directly references each callable. The current repro is `examples/ecommerce/units/pricing/discount_plus_tax.test.spec`, which semantically covers `money/round` because `apply_discount` and `apply_tax` transitively use it, yet the test body never calls `round()` directly, so generated code imports `crate::money::round::round` and trips `unused_imports` under `-D warnings`. This appears to be older molecule-test generator behavior from the M7 path, not an M12 data-seam regression, so it should be tracked as a post-M12 generator hygiene fix rather than a reason to reopen the landed M12 branch. **Fix target:** `spec-core/src/generator.rs` molecule-test import emission. **Acceptance:** the canonical ecommerce generate/check/test loop remains green, and strict consumers can run `RUSTFLAGS='-D warnings' cargo test --manifest-path examples/ecommerce/Cargo.toml` without unused-import failures.
