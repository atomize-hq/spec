...
- [ ] **Cross-library dep IMPLEMENTATION** — Implement the chosen namespace-prefixed schema (`shared::money/round`) as the M9 contract-first first cut: root-owned `[libraries]` config, repo-scoped direct library loading only, typed dep identity shared by validator/generator/export, `use <alias>::...` generation, Cargo dependency alias validation, export schema_version 3 for structured dep refs, and direct cross-library cycle detection. Explicitly out of scope for this cut: out-of-repo libraries, transitive library discovery, cross-library `.test.spec` covers, and cross-library graph-query semantics.
